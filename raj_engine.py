...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq, header):
        ...
        if train_setting == 'coco_add_semi' or train_setting == 'dvd':
            #breakpoint()
        ...
        # differences = [(i, a.item(), b.item()) for i, (a, b) in enumerate(zip(targets_label_k[0], pseudo_label_k[0])) if a != b]
        # for idx, val1, val2 in differences:
        #     print(f"Index {idx}: targets_label_k[0] has {val1} while pseudo_label_k[0] has {val2}")
...
def train_one_epoch_burnin(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        samples, targets = prefetcher.next()
        # break # stop training epoch after one batch
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}
